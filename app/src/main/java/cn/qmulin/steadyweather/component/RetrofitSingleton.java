...
import cn.qmulin.steadyweather.base.C;
...
public class RetrofitSingleton {
    ...
    public Observable<Weather> fetchWeather(String location) {
        return sApiService.mWeatherAPI(location, C.KEY)
                .flatMap(weather -> {
    ...
    public Observable<CityEntity> fetchCity(String location) {
        return sApiService.mSearchAPI(location, C.KEY,C.GROUP)
                .flatMap(Observable::just)
                .map(cityAPI -> cityAPI.mCities.get(0))
                .compose(RxUtil.io());
    }

    public Observable<String> fetchImg() {
        return sApiService.mBingImg()
